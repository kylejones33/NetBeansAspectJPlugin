...
//            System.out.println("modifiers of " + node + ": " + node.getModifiers());
            NbIconRegistry icons = (NbIconRegistry) Ajde.getDefault().getIconRegistry();

            if (node.getKind().equals(IProgramElement.Kind.ADVICE)) {
                boolean dynamic = false;
                List<IRelationship> relationships = AsmManager.lastActiveStructureModel.getRelationshipMap().get(node);
                
//                System.out.println("getting rels of "+node);
                if (relationships != null && relationships.size() == 1) {
                    IRelationship relationship = relationships.get(0);
