...
public class NbProjectProperties  {

    private final HashMap<Project, String> outputPathMap = new HashMap<Project, String>();
...
//        System.out.println("NbProjectProperties#getOutputPath");
        ...
        try {
            ...
            if (sources == null) {
                BrowserErrorHandler.handleWarning("Project contains no sources");
                return null;
            }
            ...
            if (sgroups.length == 0) {
                BrowserErrorHandler.handleWarning("No project roots???");
                return null;
            }
...
//            }
            ...
            for (SourceGroup sourceGroup : sgroups) {
                ...
                if (units.length != 0) {//unit tests haben keine unit tests
                    ...
                    URL sourceGroupRootFolderURL = sourceGroupRootFolder.toURL();
...
//                    System.out.println("outputs: ");
                    ...
                    if (outputsForSourceGroup.length == 0) {
                        BrowserErrorHandler.handleWarning("No output path???");
                        return null;
                    } else if (outputsForSourceGroup.length > 1) {
                        BrowserErrorHandler.handleWarning("More than one output path");
                        return null;
                    }
                    //outputPath = outputsForSourceGroup[0].getPath();
                    outputPath = Utilities.toFile(outputsForSourceGroup[0].toURI()).getAbsolutePath();
...
//                    sourceFolderPath = new File(sourceGroupRootFolderURL.toURI()).getAbsolutePath();
    ...
    private List<SourceGroup> sourcesNoTests() {
        ...
        Sources sources = ProjectUtils.getSources(_project);
        SourceGroup[] sgroups = sources.getSourceGroups(JavaProjectConstants.SOURCES_TYPE_JAVA);
